...
function flattenSchedule(schedule?: Record<string, string[]>): { label: string; sortKey: string }[] {
  ...
  for (const [d, arr] of Object.entries(schedule)) {
    ...
    for (const t of arr) {
      const dt = `${d} ${t}`;
      const date = new Date(`${d}T${t}:00Z`); // 並び用
...
function extractTexts(it: Submission): string[] {
  // overlay.values があればそれを優先的に採用（title/body/footer/subtitle の順で並べる）
  const vals: string[] = [];
  ...
  // 無ければ既存の message/caption/lines/title を併用
  if (vals.length === 0) {
...
export default function AuthenticatedPage() {
  ...
  const load = async (ab?: AbortSignal) => {
    setLoading(true);
    setErr(null);
    try {
      const list = await apiGet<Submission[]>("/api/admin/review/queue?status=approved", ab);
      setItems(list || []);
    } catch (e: any) {
  ...
  const title = `承認一覧（${total}${loading ? " 読み込み中…" : ""}）`;
