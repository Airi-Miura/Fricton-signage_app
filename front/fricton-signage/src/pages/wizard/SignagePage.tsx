// src/pages/wizard/SignagePage.tsx
...
export default function SignagePage({ title }: Props) {
  ...
  const [otherFiles, setOtherFiles] = useState<string[]>([]); // 画像/動画以外
  ...
  useEffect(() => {
    ...
    const qs = new URLSearchParams({
      ...
      kind: "配置型サイネージ", // ← このページはサイネージなので固定
    });
    ...
    fetch(`${BOOKED_API}?${qs.toString()}`, { signal: ctrl.signal })
      ...
      .catch(err => {
        ...
        setBooked(new Set()); // フォールバック：全て未予約
  ...
  const handleMouseEnter =
    (dayIdx: number, slotIdx: number, keyBuilder: (d: number, s: number) => string) =>
    () => {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (pickedSlots.size === 0) {
      setErr("配信時間帯を1つ以上選択してください");
      return;
    }
    if (!files || files.length === 0) {
      setErr("動画または画像ファイルを選択してください");
      return;
    }
    // 画像サイズチェック（必要なら）
    if (imgPreviews.some(p => !p.ok)) {
      setErr("画像サイズは 500px × 800px にしてください（NGの画像があります）");
      return;
    }
    // ===== 追加：クライアント側の重複検出（予約済みと被っていないか） =====
    const conflicts = Array.from(pickedSlots).filter(k => booked.has(k));
    if (conflicts.length > 0) {
      setErr("既に予約済みの時間帯が含まれています。別の時間を選んでください。");
      return;
    }

    setLoading(true);
    try {
      // byDate は pickedSlots から作成
      ...
      const res = await fetch(`${API_ROOT}/api/signage`, {
        method: "POST",
        body: fd,
      });
      ...
      alert("送信しました！");
      // 送信後：同週で再フェッチ（即グレー反映）＆選択クリア
      setAnchorDate(d => new Date(d));
      setPickedSlots(new Set());
    } catch (err: any) {
      console.error(err);
      setErr(err?.message || "送信に失敗しました。もう一度お試しください。");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div style={{ maxWidth: 1000, margin: "24px auto", padding: 16 }}>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        <div style={{ fontSize: 12, marginTop: 4 }}>
          <strong>画像サイズ指定：</strong>500px × 800px（縦）
        </div>
              ...
              {imgPreviews.map(p => (
                <div key={p.url} style={{ border: "1px solid #ddd", borderRadius: 8, padding: 8, background: "#fff" }}>
                  ...
                  <div style={{ marginTop: 4, fontSize: 12, fontWeight: 700, color: p.ok ? "#16a34a" : "#b91c1c" }}>
                    {p.ok ? "OK（500×800）" : "NG：500×800にしてください"}
                  </div>
                </div>
              ))}
            </div>
